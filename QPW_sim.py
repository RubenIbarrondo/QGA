"""
.. module:: QPW_sim.py
    :synopsis: Implements the Quantum Pair Work Optimization Algorithm (QPW).
...
def quantum_pair_work(fitness_criteria, fitness_basis=None,
                              init_population=None, cl=None, n=None,
    ...
    # Build the crossover routine unitary --> !We dont need it
    #cross_index = cl // 2
    ...
    for generation in range(generation_number):
        ...
        rho_pop = qm.rho(np.kron(rho_pop_mat,
                                 qm.rho([1], [0], [0], (2 ** a, 2 ** a)).get_matrix()), dense=True)
        # We don't perform the combination of the genetic material
...
def qpw_uqcm_test(fitness_states, samples, dirpath):
    """
    :return:
    """
    g = 10
    ...
    with open(dirpath+'/0-Notes', 'w') as notes:
        notes.write(dirpath + "\n-" * 32 + "\n" +
                    "Simulates QPW with UQCM and with fitness criteria based in this sorting order:\n")
        for i, state in enumerate(fitness_states):
            notes.write("%d.\n" % i)
    ...
    for trial in range(samples):
        ...
        rho_final, ft = quantum_pair_work(criteria, fitness_basis=uu,
...
def qpw_simulation(c, n, number_of_first_Hp, number_of_last_Hp):
    ...
    big_dirs = ["out_QPW_uqcm_"+str(n)+"_"+str(c), "out_QPW_inits_"+str(n)+"_"+str(c), "out_QPW_ups_"+str(n)+"_"+str(c)]
    ...
    for i in range(number_of_first_Hp, number_of_last_Hp):
        ...
        for j in range(number_of_initial_populations):
            ...
            np.save(big_dirs[-2] + "/" + pathname + '/initial_population_{:03d}'.format(j), np.array(v, dtype=np.complex64))

            # QPW uqcm
            rho_final, ft = quantum_pair_work(fitness_criteria=criteria, fitness_basis=up,
...
if __name__ == '__main__':
    t1 = time()
    qpw_simulation(c=3, n=4, number_of_first_Hp=0, number_of_last_Hp=1)
    print(time() - t1)
